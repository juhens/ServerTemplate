...
namespace ServerCore.Job
{
    ...
    public class Job : JobBase
    {
        ...
        {
            if (!Pool.TryTake(out var job)) job = new Job();
            job._action = action;
    ...
    {
        private static readonly ConcurrentBag<Job<T1>> Pool = new();
        private Job() { }
        ...
        {
            if (!Pool.TryTake(out var job)) job = new Job<T1>();
            job._action = action;
            job._t1 = t1;
            job.Priority = priority;
            job.Cancel = false;
    ...
    {
        private static readonly ConcurrentBag<Job<T1, T2>> Pool = new();
        private Job() { }
        ...
        {
            if (!Pool.TryTake(out var job)) job = new Job<T1, T2>();
            job._action = action;
            job._t1 = t1;
            job._t2 = t2;
            job.Priority = priority;
    ...
    {
        private static readonly ConcurrentBag<Job<T1, T2, T3>> Pool = new();
        private Job() { }
        ...
        {
            if (!Pool.TryTake(out var job)) job = new Job<T1, T2, T3>();
            job._action = action;
            job._t1 = t1;
            job._t2 = t2;
            job._t3 = t3;
    ...
    {
        private static readonly ConcurrentBag<Job<T1, T2, T3, T4>> Pool = new();
        private Job() { }
        ...
        {
            if (!Pool.TryTake(out var job)) job = new Job<T1, T2, T3, T4>();
            job._action = action;
            job._t1 = t1;
            job._t2 = t2;
            job._t3 = t3;
    ...
    {
        private static readonly ConcurrentBag<Job<T1, T2, T3, T4, T5>> Pool = new();
        private Job() { }
        ...
        {
            if (!Pool.TryTake(out var job)) job = new Job<T1, T2, T3, T4, T5>();
            job._action = action;
            job._t1 = t1;
            job._t2 = t2;
            job._t3 = t3;
